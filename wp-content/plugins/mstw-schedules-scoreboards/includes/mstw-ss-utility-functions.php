 ...
 * 3.1. mstw_ss_get_sb_defaults - returns the default scoreboard options
 *		(mstw_ss_sb_options[])
 * 3.2. mstw_ss_get_venue_defaults  - returns the default venue options 
 *		(mstw_ss_venue_options[])
 ...
 * 19. mstw_ss_admin_notice - Displays all admin notices; callback for 
 *		admin_notices action
 * 20. mstw_ss_add_admin_notice - Adds a notice to the list for display by 
 *		mstw_ss_admin_notice
 * 21. mstw_find_game_venue - Finds the venue (CPT object) for a game (CPT object)
 * 22. mstw_ss_build_media_links - Builds a game's media links
 * 23. mstw_ss_build_game_sport - Builds the SPORT entry when 'show_sport' is set
 * 24. mstw_ss_build_game_time_results - builds the game time entry
 *	 
 *--------------------------------------------------------------------------*/
...
if ( !function_exists( 'mstw_ss_add_css_to_head' ) ) {			
	function mstw_ss_add_css_to_head( ) {	
		...
		//
		// SCHEDULE TABLE SHORTCODE & WIDGET
		//
		
		// Header
		echo ".mstw-ss-table thead tr th, .mstw-ss-sw-tab-head th { \n";
		...
		// All rows
		echo ".mstw-ss-table tbody tr td { \n";
			echo mstw_build_css_rule( $colors, 'ss_tbl_border_color', 'border-color' );
		echo "} \n";
		
		
		// Odd rows
		echo ".mstw-ss-table tbody tr:nth-child(odd) { \n";
		...
		echo ".mstw-ss-table tbody tr:nth-child(odd) a:link, .mstw-ss-table tbody tr:nth-child(odd) a:visited { \n";
			echo mstw_build_css_rule( $colors, 'ss_tbl_odd_text_color', 'color' );
			echo mstw_build_css_rule( $colors, 'ss_tbl_odd_bkgd_color', 'background-color' );
		echo "} \n";
		
		// Even rows
		echo ".mstw-ss-table tbody tr:nth-child(even) { \n";
		...
		echo ".mstw-ss-table tbody tr:nth-child(even) a:link, .mstw-ss-table tbody tr:nth-child(even) a:visited  { \n";
			echo mstw_build_css_rule( $colors, 'ss_tbl_even_text_color', 'color' );
			echo mstw_build_css_rule( $colors, 'ss_tbl_even_bkgd_color', 'background-color' );
		echo "} \n";
		
		// Home rows
		echo ".mstw-ss-home td, .mstw-ss-home td { \n";
		...
		echo "} \n";
				...
				echo ".mstw-ss-table tbody tr.mstw-ss-home:nth-child(even) a, .mstw-ss-table tbody tr.mstw-ss-home:nth-child(odd) a { \n";
			echo mstw_build_css_rule( $colors, 'ss_tbl_home_text_color', 'color' );
			echo mstw_build_css_rule( $colors, 'ss_tbl_home_bkgd_color', 'background-color' );
		echo "} \n";
		...
		//
		echo ".mstw-cdt-block .mstw-ss-cdt-dtg { \n";
			echo mstw_build_css_rule( $colors, 'ss_cdt_game_time_color', 'color' );
		echo "} \n";
		
		echo ".mstw-ss-cdt-opp-loc .mstw-ss-cdt-opponent, .mstw-ss-cdt-opp-loc .mstw-ss-cdt-opponent a, .mstw_ss_cdt_widget_class .mstw-ss-cdt-opp-loc .mstw-ss-cdt-opponent a { \n";
			echo mstw_build_css_rule( $colors, 'ss_cdt_opponent_color', 'color' );
		echo "} \n";
		
		echo ".mstw-ss-cdt-opp-loc .mstw-ss-cdt-location, mstw-ss-cdt-opp-loc .mstw-ss-cdt-location a { \n";
		...
		echo ".ss-slider .full-schedule-link  a, .ss-slider .full-schedule-link  a:visited { \n";
			echo mstw_build_css_rule( $colors, 'ss_sldr_hdr_text_color', 'color' );
		echo "} \n";
		...
		echo ".game-block .opponent, .game-block .opponent a, .game-block .opponent a:hover, .game-block .opponent a:visited { \n";
			echo mstw_build_css_rule( $colors, 'ss_sldr_game_opponent_color', 'color' );
		echo "} \n";
		
		echo ".game-block .location, .game-block .location a, .game-block .location a:hover, .game-block .location a:visited { \n";
		...
		echo ".game-block .links, .game-block .links a, .game-block .links a:hover, .game-block .links a:visited  { \n";
...
if ( !function_exists( 'mstw_ss_build_team_link' ) ) {
	function mstw_ss_build_team_link( $post, $link_type='no-link', $team='opponent' ) {
		...
		if ( $link_type == 'game-page' ) {
			...
			//mstw_log_msg( 'in mstw_ss_build_team_link ...' );
		...
		// else it's a home game, so if there's an entry in the TEAMS DB, use it
			...
			if( !empty( $sched_id ) ) {  //this should never, ever be empty
				...
				if( null !== $home_team_obj ) {
					...
					// home_team_object is null
					mstw_log_msg( 'home_team_object is null ... this should not happen');
					$location_entry = sprintf( __( 'schedule %s does not appear to have a team assigned.', 'mstw_schedules_scoreboards' ), $sched_slug  );
				}
...
if ( !function_exists( 'mstw_ss_build_rosters_list' ) ) {
	function mstw_ss_build_rosters_list( ) {
		...
		$terms = get_terms( 'mstw_tr_team', array( 'hide_empty' => 0 ) );
...
if ( !function_exists( 'mstw_ss_build_countdown' ) ) {
	function mstw_ss_build_countdown( $attribs ) { 
		...
		if ( ! $have_games ) {
			...
			$ret_str .= "<div class = 'mstw-cdt-block mstw-cdt-block_$sched $home_css_tag'>";
			...
			$dtg_span = "<div class='mstw-ss-cdt-dtg mstw-ss-cdt-dtg_$sched'>";
			$ret_str .= $dtg_span . $dtg_str . '</div>';
			...
			$ret_str .= "<div class='mstw-ss-cdt-opp-loc'>";
			$ret_str .= "<span class='mstw-ss-cdt-opponent'>$opponent_entry</span><span class='mstw-ss-cdt-location'> @$location_entry</span>";
			$ret_str .= '</div>';
			
			// Add the intro text set in shortcut arg or widget setting
			$ret_str .= "<div class='mstw-ss-cdt-intro'>$intro</div>";
			
			// Add the countdown
			settype($game_dtg, 'integer');
			$time_diff = mstw_ss_time_difference( $game_dtg - $current_dtg );
	
			$ret_str .= "<div class='mstw-ss-cdt-countdown'>$time_diff</div>";
			$ret_str .= '</div>';
		}
...
if ( !function_exists ( 'mstw_ss_admin_notice' ) ) {
	function mstw_ss_admin_notice( ) {
		...
		//mstw_log_msg( 'calling mstw_admin_notice ... ' );
		//this one's in mstw_utility_functions: need it for validate_url
		mstw_admin_notice( );
...
// ARGUMENTS:
// 		$game - a game OBJECT
//		$options - the display options for the game
// RETURNS:
//		HTML for media links
//
if ( !function_exists ( 'mstw_ss_build_media_links' ) ) {
	function mstw_ss_build_media_links( $game, $options ) {
		//mstw_log_msg( "in mstw_ss_build_media_links( ) ..." );
	...
	} //End: mstw_ss_build_media links( )
}

//--------------------------------------------------------------------------------------
// 23. mstw_ss_build_game_sport - Builds the SPORT entry when 'show_sport' is set
//		Called by mstw_ss_build_schedule_table() and mstw_ss_build_game_block() for
//		schedule sliders
// ARGUMENTS:
// 	$game - a game OBJECT
// RETURNS
//	HTML for the sport string "Baseball vs" or "Football @"
//
if ( !function_exists ( 'mstw_ss_build_game_sport' ) ) {
	function mstw_ss_build_game_sport( $game ) {
		//mstw_log_msg( "in mstw_ss_build_game_sport( ) ... ");
		//return variable
		$sport = null;
		
		//from the game, find the game's schedule
		...
		if ( null !== $sched_obj ) {
			...
			if ( null !== $team_obj ) {
				...
				$sport_obj = get_page_by_path( $sport_slug, OBJECT, 'mstw_ss_sport' );
				if ( null != $sport_obj ) {
		...
		/* //This is debug stuff
		if ( null === $sched_obj ) {
			mstw_log_msg( '$sched_obj === null' );
		}
		if ( null === $team_obj ) {
			mstw_log_msg( '$team_obj === null' );
		}
		//End debug stuff */
		
		$vs_or_at = __( 'vs', 'mstw-schedules-scoreboards' );
		if ( !get_post_meta($game->ID, 'game_is_home_game', true ) ) {
			$vs_or_at = __( '@', 'mstw-schedules-scoreboards' );
		}
...
// ARGUMENTS:
// 	$game - a game object
//	$time_format - the game time format as a PHP time format string
// RETURNS
//	HTML for the game time string
//
if ( !function_exists ( 'mstw_ss_build_game_time_results' ) ) {
	function mstw_ss_build_game_time_results( $game, $time_format ) {
		//mstw_log_msg( "in mstw_ss_build_game_time_results( ) ... " );
		$ret_str = '';
		
		$game_result = get_post_meta( $game->ID, 'game_result', true); 
		
		if ( $game_result != '' ) {
			// If there is a game result, return it and we're done
			$ret_str .=  $game_result;
		}
		else {	
			...
			if ( $time_is_tba != '' ) {	
				//Time is TBA. Return it and we're done
				$ret_str .= $time_is_tba;
			}
			else {	
				...
				$ret_str .= date( $time_format, $unix_dtg );
			}	
		}
		
		return $ret_str;

	} //End: mstw_ss_build_game_time_results
}
?>